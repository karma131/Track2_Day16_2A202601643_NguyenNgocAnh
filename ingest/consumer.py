...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    try:
        # Parse cả batch JSON trong DuckDB, tránh một round trip cho mỗi row.
        con.execute(
            f"""insert into {TABLE}
            select
                value ->> '$.event_id',
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            write_batch(con, batch)           # ghi idempotent trước
            maybe_crash(batch_no, crash_at)   # crash -> batch sẽ được phát lại
